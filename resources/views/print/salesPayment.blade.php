...
<head>
    ...
    <title>Print Pelunasan Piutang - {{ $salesPayment->code }}</title>
...
<body onload="window.print()">
    ...
    <div class="container my-4">
        <div class="header-title">
            <h3><strong>FAKTUR PELUNASAN PIUTANG</strong></h3>
        </div>

        <p><strong>Kode:</strong> {{ $salesPayment->code }}</p>
        <p><strong>Tanggal:</strong>
            {{ \Carbon\Carbon::parse($salesPayment->date)->locale('id')->translatedFormat('l, d/m/Y') }}</p>
        <p><strong>Customer:</strong> {{ $salesPayment->contact->prefix }} {{ $salesPayment->contact->name }}
            ({{ $salesPayment->contact->organization_name }})</p>
        ...
        <div class="row mt-3">
            ...
            <div class="col-4 text-center">
                ...
                <div class="signature">
                    <br><br><br>
                    <p><strong>{{ $salesPayment->user->name ?? '-' }}</strong></p>
                </div>
            </div>
        </div>
    </div>
