...
@section('content')
    <div class="container-fluid py-0 px-0">
        <h1 class="h3 mb-3"><strong>Edit Timbangan Tetes</strong></h1>

        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('mollases.update', $mollase->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="created_at" id="created_at"
                            ...
                            value="{{ $mollase->created_at }}" required autofocus>
                        @error('created_at')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="bruto" id="bruto"
                            class="form-control @error('bruto') is-invalid @enderror" value="{{ $mollase->bruto }}"
                            required>
                        @error('bruto')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="tarra" id="tarra"
                            class="form-control @error('tarra') is-invalid @enderror" value="{{ $mollase->tarra }}"
                            required>
                        @error('tarra')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="netto" id="netto"
                            class="form-control @error('netto') is-invalid @enderror" value="{{ $mollase->netto }}"
                            readonly>
                        @error('netto')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
