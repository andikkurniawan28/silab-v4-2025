...
@section('content')
    <div class="container-fluid py-0 px-0">
        <h1 class="h3 mb-3"><strong>Daftar Keliling Proses</strong></h1>

        @if (Auth()->user()->role->akses_tambah_keliling_proses)
            <div class="d-flex justify-content-between align-roles-center mb-3">
                <a href="{{ route('keliling_proses.create') }}" class="btn btn-primary">
    ...
    <script>
        $(function() {
            $('#analysesTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('keliling_proses.index') }}",
                order: [
                    [0, 'desc']
                ],
                columns: [{
